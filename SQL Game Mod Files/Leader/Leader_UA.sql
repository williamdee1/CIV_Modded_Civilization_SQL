...
-- The standard naming convention follows: TRAIT_LEADER_TRAITNAME

-- Configuring a Unique Ability for your custom leader is entirely optional, but it is typically considered appropriate for balance to configure one, such that your custom leader matches those from the base games in terms of complexity, both for flavour and gameplay balance (in my opinion).
...
-- The Unique Ability this example creates will be applied to two specific UnitType entities - the Builder and the Settler. As such, because these two - and no others - do not exist as their own class of unit, we need to ring-fence them accordingly. The below section does this for us, by inserting our new CLASS_BUILDER_SETTLER class as an ABILITY_CLASS - such that we can tie the new Ability to them.
-----------------------------------------------
...
-- Firstly, it associates the UNIT_BUILDER and the UNIT_SETTLER to our new class - CLASS_BUILDER_SETTLER.
...
INSERT INTO TypeTags
		(Type,											Tag						)
VALUES	('UNIT_WEREJAGUAR',								'CLASS_BUILDER_SETTLER'	),
		('UNIT_SETTLER',								'CLASS_BUILDER_SETTLER'	),
		('UNIT_BUILDER',								'CLASS_BUILDER_SETTLER'	),
...
-- The below entry tie the effective modifiers to the UnitAbilityType. A modifier is the top-level item that instructs the game to perform a deviation away from the base ruleset in some way. The possibilities are endless, so these notes make no attempt to cover what can be done - only to instruct against the logic, step by step.
