...
-- In this example mod, the string 'INSERT OR Replace INTO' is used as a way to flexibly negotiate the possibility that the IMPROVEMENT_COLOSSAL_HEAD already exists - as this particular improvement is, in fact, configured in the base game. For info, this particular improvement is one that can be built by the Suzerain of a specific City-State. By using this element, we seamlessly eliminate the need to configure anything to have the 3D model display in-game.
-----------------------------------------------		

INSERT OR Replace INTO Types	
		(Type,													Kind						)
VALUES	('TRAIT_CIVILIZATION_IMPROVEMENT_OLMEC_COLOSSAL_HEAD',	'KIND_TRAIT'				),
		('IMPROVEMENT_OUTBACK_STATION',							'KIND_IMPROVEMENT'			);

-----------------------------------------------
-- CivilizationTraits

-- This defines the civilization to which the TraitType is applied (i.e. which civilization gets the Unique Ability). This is a simple matter of referencing the custom CivilizationType defined in Civilization_Config.sql and using the TraitType defined in this document.
-----------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------
DELETE FROM Improvements WHERE ImprovementType = 'IMPROVEMENT_OUTBACK_STATION';
...
VALUES	
		(
			...
			'LOC_IMPROVEMENT_OLMEC_COLOSSAL_HEAD_NAME',
			'LOC_IMPROVEMENT_OLMEC_COLOSSAL_HEAD_DESCRIPTION',
			...
			1,
			0,
			'TRAIT_CIVILIZATION_IMPROVEMENT_OLMEC_COLOSSAL_HEAD',
			"TECH_POTTERY",
			-----------------
			-- Plunder Info
			-----------------
...
-----------------------------------------------------------------------------------
DELETE FROM Improvement_ValidTerrains WHERE ImprovementType = 'IMPROVEMENT_OUTBACK_STATION';

INSERT OR REPLACE INTO Improvement_ValidTerrains
			(ImprovementType,								TerrainType				)
VALUES 		('IMPROVEMENT_OUTBACK_STATION',					'TERRAIN_GRASS'			),
			('IMPROVEMENT_OUTBACK_STATION',					'TERRAIN_GRASS_HILLS'	),
			('IMPROVEMENT_OUTBACK_STATION',					'TERRAIN_PLAINS'		),
			('IMPROVEMENT_OUTBACK_STATION',					'TERRAIN_PLAINS_HILLS'	),
			('IMPROVEMENT_OUTBACK_STATION',					'TERRAIN_TUNDRA'		),
...
VALUES 		('IMPROVEMENT_OUTBACK_STATION',			'FEATURE_FOREST'	);
...
VALUES 		('IMPROVEMENT_OUTBACK_STATION',			'UNIT_BUILDER'		);
...
-----------------------------------------------------------------------------------	
DELETE FROM Improvement_YieldChanges WHERE ImprovementType = 'IMPROVEMENT_OUTBACK_STATION';

INSERT or REPLACE INTO Improvement_YieldChanges	
		(ImprovementType,					YieldType,						YieldChange		)
VALUES	('IMPROVEMENT_OUTBACK_STATION',		'YIELD_CULTURE',						2				),
		('IMPROVEMENT_OUTBACK_STATION',		'YIELD_PRODUCTION',						2				);		
...
VALUES	('IMPROVEMENT_OUTBACK_STATION',			'OLMEC_COLOSSAL_HEAD_GRASSMOUNT'		),
		('IMPROVEMENT_OUTBACK_STATION',			'OLMEC_COLOSSAL_HEAD_PLAINSMOUNT'		);

INSERT INTO Adjacency_YieldChanges
		(ID,								Description,	YieldType,			YieldChange,	TilesRequired,		AdjacentTerrain			)
VALUES	('OLMEC_COLOSSAL_HEAD_GRASSMOUNT',	'Placeholder',	'YIELD_PRODUCTION',		1,				1,					'TERRAIN_GRASS_MOUNTAIN'		),
		('OLMEC_COLOSSAL_HEAD_PLAINSMOUNT',	'Placeholder',	'YIELD_PRODUCTION',		1,				1,					'TERRAIN_PLAINS_MOUNTAIN'		);
...
-- The starting point to search in the base game XML is to look-up an existing CivilizationType (e.g. CIVILIZATION_FRANCE) and note their TRAIT_CIVILIZATION_ that is neither the Unique Ability nor the Unique Unit. In France's case, it is TRAIT_CIVILIZATION_IMPROVEMENT_CHATEAU. Searching that will lead you to the ModifierId entry (or entries) that grant the various bonuses associated with that TraitType. You can apply this logic to do a cascading look-up all the way down, as long as you appreciate that you must follow every 'route' down which you are led to build up a picture of all of the cross-references/dependencies.
-------------------------------------
INSERT INTO TraitModifiers			
		(TraitType,													ModifierId								)
VALUES	('TRAIT_CIVILIZATION_IMPROVEMENT_OLMEC_COLOSSAL_HEAD', 		'OLMEC_COLOSSAL_HEAD_GREAT_ENGINEER'		);
...
VALUES	('OLMEC_COLOSSAL_HEAD_GREAT_ENGINEER',				'MODIFIER_PLAYER_CITIES_ATTACH_MODIFIER',				'OLMEC_4_COLOSSAL_HEAD'		),
		('OLMEC_COLOSSAL_HEAD_GREAT_ENGINEER_POINT',			'MODIFIER_PLAYER_ADJUST_GREAT_PERSON_POINTS',			null						);
...
-- The ModifierType governs the actual 'values' that are accepted within the Name column, which in turn governs the values that are accepted within the value column. I expect the below example data will make those connections clear. You may note that the below section ties together two separate ModifierId elements by referencing the value of OLMEC_COLOSSAL_HEAD_GREAT_ARTIST_POINT to populate the value of OLMEC_COLOSSAL_HEAD_GREAT_ARTIST.
-------------------------------------
INSERT INTO ModifierArguments
		(ModifierId,											Name,						Value										)
VALUES	('OLMEC_COLOSSAL_HEAD_GREAT_ENGINEER',					'ModifierId',				'OLMEC_COLOSSAL_HEAD_GREAT_ENGINEER_POINT'	),
		('OLMEC_COLOSSAL_HEAD_GREAT_ENGINEER_POINT',			'GreatPersonClassType',		'GREAT_PERSON_CLASS_ENGINEER'					),
		('OLMEC_COLOSSAL_HEAD_GREAT_ENGINEER_POINT',			'Amount',					2											);
...
-- REQUIREMENT_TEST_ALL: Every listed criteria must return 'true' for an overall value of 'true'
-- REQUIREMENT_TEST_ANY: Any one listed criteria must return 'true' for an overall value of 'true'
-------------------------------------
...
VALUES	('OLMEC_4_COLOSSAL_HEAD_REQUIREMENT', 			'CollectionType',			'COLLECTION_CITY_PLOT_YIELDS'	),
		('OLMEC_4_COLOSSAL_HEAD_REQUIREMENT', 			'Count',					2								),
		('OLMEC_4_COLOSSAL_HEAD_REQUIREMENT', 			'RequirementSetId',			'PLOT_HAS_OLMEC_COLOSSAL_HEAD'	),
		('PLOT_HAS_OLMEC_COLOSSAL_HEAD_REQUIREMENT', 	'ImprovementType',			'IMPROVEMENT_OUTBACK_STATION'	);